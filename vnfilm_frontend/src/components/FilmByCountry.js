...
const FilmByCountry = () => {
  ...
  const handleChangePage = (event, newPage) => {
    history.replace(`/country/${country_id}/${newPage}`);
  ...
  useEffect(() => {
    const fetchByCountry = async () => {
      try {
        ...
        const responseByCountry= await filmApi.getByCountry(country_id, params);
        if (responseByCountry) {
          setFilm(responseByCountry);
          setIsSuccess(true);
        }
      } catch (error) {
        setIsSuccess(false);
          ...
          <div>
            <div className={classes.list}>
              <div className={classes.title}>
                <h1 className={classes.text}>Phim {film.country_name}</h1>
              </div>
